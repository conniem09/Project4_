...
/* Zachary driving */
/* Creates new supplementary page table entry and adds it to 
   thread's supplementary page table. */
void 
...
/* Cindy driving */
/* Looks up the supplementary page table entry for upage in the 
   owner's supplementary page table. */
struct supp_pte *
spte_lookup (void *upage, struct thread *owner)
{
  ...
  spte.upage = upage;
  e = hash_find (&owner->supp_page_table, &spte.supp_elem);
...
{ 
  ...
  if (pagedir_set_page (thread_current ()->pagedir, upage, kpage, true)) 
    {
      create_fte (upage, kpage);
      struct supp_pte *spte = spte_lookup (upage, thread_current ());
...
/* Zachary stops driving */

/* Prints a supplementary page table entry.
   For debugging. */
void 
